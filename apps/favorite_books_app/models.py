from __future__ import unicode_literals
from django.db import models
import re

class UserManager(models.Manager):
    def register_user_validator(self, postData):
        errors = {}
        EMAIL_REGEX = re.compile(r'^[a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$')
        
        if len(postData['first_name']) < 2:
            errors['first_name'] = 'First name must be longer than 2 letters'
        if len(postData['last_name']) < 2:
            errors['last_name'] = "Last name must be longer than 2 letters"
        if not EMAIL_REGEX.match(postData['reg_email']):
            errors['email'] = 'Invalid email format'
        if len(postData['new_password']) < 8 or postData['new_password'] != postData['confirm_password']:
            errors['new_password'] = 'Invalid password'
        return errors
    
    def login_user_validator(self, postData):
        login_errors = {}
        EMAIL_REGEX = re.compile(r'^[a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$')
        if not EMAIL_REGEX.match(postData['email']):
            login_errors['email'] = 'Invalid email format'
        if len(postData['user_password']) < 8:
            login_errors['password'] = 'Invalid password'
        return login_errors

    def add_book_validator(self, postData):
        book_errors = {}
        if (len(postData['book_title']) < 5):
            book_errors['desc'] = 'title must be longer than 5 characters'
        if (len(postData['book_description']) < 5):
            book_errors['desc'] = 'Description must be longer than 5 characters'
        return book_errors
    
    def update_description_validator(self, postData):
        update_errors = {}
        if (len(postData['edit_description']) < 5):
            update_errors['desc'] = 'Description must be longer than 5 characters'
        return update_errors

class User(models.Model):
    first_name = models.TextField(max_length=255)
    last_name = models.TextField(max_length=255)
    email = models.TextField(max_length=255)
    password = models.TextField(max_length=255)
    create_date = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now = True)
    objects = UserManager()

class Book(models.Model):
    title = models.TextField(max_length=255)
    description = models.TextField()
    uploaded_by = models.ForeignKey(User, related_name='books_uploaded')
    users_who_like = models.ManyToManyField(User, related_name='liked_books')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now = True)
    objects = UserManager()
